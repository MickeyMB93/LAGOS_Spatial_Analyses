...
## 1) What is the correlation between Secchi Disk Depth and Chlorophyll a for sites with at least 200 observations? Here, I just want a plot of chla vs secchi for all sites 
...
mean_values_sec200 <- secchi1_200 %>%
  # Take summary by lake id
  group_by(lagoslakeid) %>%
  # take mean chl_a per lake id
  summarize(mean_secchi1=mean(secchi,na.rm=T))
  

 spatial_secchi_200 <- inner_join(spatial_lakes, mean_values_sec200,
                          by='lagoslakeid') %>%
  arrange(mean_values_sec200) %>%
  mapview(., zcol = 'mean_secchi1')

## mapview(spatial_secchi_200, zcol = 'mean_secchi1')
```
